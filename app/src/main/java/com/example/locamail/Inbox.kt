...
class Inbox : ComponentActivity() {
    ...
    fun inboxContent(scope: CoroutineScope, drawerState: DrawerState) {
        ...
        Scaffold(
            ...
            Column(
                ...
                LazyColumn {
                    items(emails) { email ->
                        if (email.received && !email.spam && !email.deleted) {
                            EmailItemInbox(email = email) {
    ...
    @Composable
    fun EmailItemInbox(email: EmailModel, onItemClick: () -> Unit) {
        ...
        Row(
            ...
            Icon(
                imageVector = if(email.read) Icons.Default.MarkEmailRead else Icons.Default.MarkEmailUnread,
                contentDescription = "Email Icon",
                modifier = Modifier.size(24.dp)
            )
            Spacer(modifier = Modifier.width(16.dp))
            Icon(
                imageVector = if(email.important) Icons.Default.Label else Icons.Default.LabelOff,
